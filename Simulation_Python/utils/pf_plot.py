...
def pfPlot(t_all, data, ax, fig, Ts, color='blue', ifsave=False):
    ...
    def updateLines(i):
        ...
        ax.view_init(elev=10.+i/3, azim=i/2)
    ...
    def ini_plot():
        ...
        data_i = data[1*numFrames]
        time = t_all[1*numFrames]
        z,y,x = data_i.nonzero()
        cube = ax.scatter(z, y, x, zdir='z', c=data_i[z,y,x], cmap=plt.cm.rainbow, marker='o')
    ...
    ifsave =1
    if (ifsave):
        line_ani.save('Gifs/Raw/pf.gif', dpi=80, writer='imagemagick', fps=25)
