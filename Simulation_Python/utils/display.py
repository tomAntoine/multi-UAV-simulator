...
def makeAllFigures(data, pos_obs):
    ...
    plt.figure()
    for id_quad in (data):
        plt.plot(time, data[id_quad]["dist2agt"], color = data[id_quad]["color"])
    min_dist = 100
    for id_quad in (data):
        min_dis_i = min(data[id_quad]["dist2agt"])
        if min_dis_i < min_dist:
            min_dist = min_dis_i
    plt.plot(time,[min_dist for t in time], '--', color = 'red')
    plt.grid(True)
    plt.legend(id_list+['min dist: '+str(round(min_dist,3))+'m'])
    plt.xlabel('Time (s)')
    ...
    plt.figure()
    for id_quad in (data):
        plt.plot(time, data[id_quad]["dist2obs"], color = data[id_quad]["color"])
    min_dist = 100
    for id_quad in (data):
    ...
    if id_trackers :
        ...
        for id_quad in (id_trackers):
            plt.plot(time, data[id_quad]["dist2target"], color = data[id_quad]["color"])
        plt.plot(time, [1 for t in time], '--', color = 'red')
        plt.grid(True)
        plt.legend(id_trackers+['threshold: 1m'])
        plt.xlabel('Time (s)')
        plt.ylabel('Distance (m)')
